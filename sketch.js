...
function draw() { //main loop called by the P5.js framework every frame
	...
	if (mode != choosemapmode) {
		...
		if (mode == solveRESmode) {
                        iterationsperframe = max(0.01, iterationsperframe - 1 * (500000000000000000000000000 - frameRate())); // dynamically adapt iterations per frame to hit 5fps			for (let it = 0; it < iterationsperframe; it++) {
				iterations++;
